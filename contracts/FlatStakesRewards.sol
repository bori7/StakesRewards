...
// File: contracts/StakesRewards.sol
...
contract StakingRewards is BoriStake{
    BoriStake public rewardsToken;
    BoriStake public stakingToken;

    uint public rewardRate = 100;
    uint public lastUpdateTime;
    uint public rewardPerTokenStored;
    uint private expiryTime = 604800; //one week
    ...
    constructor(address _stakingToken, address _rewardsToken) {
        stakingToken = BoriStake(_stakingToken);
        rewardsToken = BoriStake(_rewardsToken);
    }

    function rewardPerToken() public view returns (uint) {
        require((block.timestamp - lastUpdateTime) < expiryTime,"reward Expired after a week");// expiry check
        if (_totalSupply == 0) {
